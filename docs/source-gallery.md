# GPT Image 2 Prompt Search Index / 分类索引

The category links use `q=<category>` because the Model Prompt Lab search endpoint supports semantic matching for GPT Image 2 cases.

分类链接统一使用 `q=<分类词>`。该检索入口支持语义召回，可以直接用原有分类词找到对应 GPT Image 2 case。
